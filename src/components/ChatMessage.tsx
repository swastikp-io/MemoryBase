...
import { Copy, Check, Bot, ThumbsUp, ThumbsDown, Globe, Edit2 } from "lucide-react";
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ id, role, content, images, isGenerating, isSearchingWeb, onEdit }) => {
  ...
  const markdownComponents = {
    ...
    blockquote({ children, ...props }: any) {
      return (
        <blockquote className="my-5 border-l-[3px] border-[var(--accent-primary)]/60 bg-[var(--accent-primary)]/5 py-2.5 pl-4 pr-3 rounded-r-lg text-text-secondary italic" {...props}>
          {children}
        </blockquote>
      );
    },
    ...
    code({ className, children, ...props }: any) {
      ...
      if (!match && !hasNewline) {
        return (
          <code className="rounded-md bg-black/10 dark:bg-white/10 px-[0.35rem] py-[0.15rem] text-[0.9em] font-mono text-[var(--accent-primary)] font-medium whitespace-nowrap" {...props}>
            {children}
          </code>
        );
      }
        ...
        {isUser ? (
          <div className="flex justify-end w-full group relative">
            <div className="max-w-[90%] sm:max-w-[70%] flex flex-col items-end">
              {isEditing ? (
                <div className="w-full bg-[var(--accent-primary)] p-4 rounded-3xl rounded-tr-md shadow-sm min-w-[280px]">
                  <textarea
                    autoFocus
                    value={editValue}
                    onChange={(e) => setEditValue(e.target.value)}
                    onKeyDown={handleKeyDown}
                    className="w-full bg-transparent text-[var(--accent-fg)] outline-none resize-none overflow-hidden min-h-[60px] scrollbar-hide text-base leading-relaxed placeholder:text-[var(--accent-fg)]/50"
                    rows={Math.max(2, editValue.split('\n').length)}
                  />
                  <div className="flex justify-end gap-2 mt-3">
                    <button 
                      onClick={() => { setIsEditing(false); setEditValue(content); }} 
                      className="px-3 py-1.5 text-xs font-medium text-[var(--accent-fg)]/80 hover:text-[var(--accent-fg)] transition-colors bg-black/10 rounded-full"
                    >
                      Cancel
                    </button>
                    <button 
                      onClick={handleEditSubmit} 
                      className="px-3 py-1.5 text-xs font-medium text-[var(--accent-primary)] bg-[var(--accent-fg)] hover:opacity-90 transition-opacity rounded-full"
                    >
                      Save & Submit
                    </button>
                  </div>
                ...
                <div className="bg-[var(--accent-primary)] text-[var(--accent-fg)] px-5 py-3 rounded-3xl text-base whitespace-pre-wrap shadow-sm rounded-tr-md">
              ...
              {!isEditing && !isGenerating && (
                <div className="flex items-center gap-1 mt-1.5 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-150">
                  <button onClick={handleCopy} className="p-1.5 text-text-secondary hover:text-text-primary hover:bg-black/5 dark:hover:bg-white/10 rounded-md transition-colors" title="Copy">
                    {isCopied ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
                  </button>
                  {onEdit && (
                    <button onClick={() => setIsEditing(true)} className="p-1.5 text-text-secondary hover:text-text-primary hover:bg-black/5 dark:hover:bg-white/10 rounded-md transition-colors" title="Edit">
          ...
          <div className="flex flex-col group w-full text-text-primary text-base leading-relaxed">
            <div className="markdown-body w-full">
              ...
              {isSearchingWeb && (
                <div className="flex items-center gap-2 mb-3 text-[13px] font-medium text-[var(--accent-primary)] bg-[var(--accent-primary)]/10 px-3 py-1.5 rounded-full w-max border border-[var(--accent-primary)]/20 shadow-sm animate-pulse">
            ...
            {!isGenerating && content && (
              <div className="flex items-center gap-2 mt-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-150">
                <button
                  onClick={handleCopy}
                  className="p-1.5 text-text-secondary hover:text-text-primary hover:bg-black/5 dark:hover:bg-white/10 rounded-md transition-colors flex items-center gap-1.5"
                  title="Copy"
                >
                  {isCopied ? (
                    <>
                ...
                <button
                  className="p-1.5 text-text-secondary hover:text-text-primary hover:bg-black/5 dark:hover:bg-white/10 rounded-md transition-colors flex items-center"
                  title="Good response"
                >
