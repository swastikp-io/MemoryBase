...
interface ChatContextType {
  ...
  updateReasoningState: (chatId: string, messageId: string, reasoning: Partial<ReasoningPhase>) => void;
...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateChatTitle = (chatId: string, newTitle: string, isGenerated: boolean = false, isCustom: boolean = false) => {
    setChats(prev => prev.map(chat => {
      if (chat.id === chatId) {
        if (isGenerated && (chat.autoGenerated || chat.manualTitle)) {
          return chat;
        }
        return {
          ...chat,
          title: newTitle,
          autoGenerated: isGenerated ? true : chat.autoGenerated,
          manualTitle: isCustom ? true : chat.manualTitle,
          updatedAt: Date.now(),
        };
      }
      return chat;
  ...
  const updateReasoningState = (chatId: string, messageId: string, reasoningUpdate: Partial<ReasoningPhase>) => {
    setChats(prev => prev.map(chat => {
      if (chat.id === chatId) {
        return {
          ...chat,
          messages: chat.messages.map(msg => 
            msg.id === messageId 
              ? { 
                  ...msg, 
                  reasoning: { 
                    ...(msg.reasoning || { status: "", isComplete: false }), 
                    ...reasoningUpdate 
                  }
                } 
              : msg
